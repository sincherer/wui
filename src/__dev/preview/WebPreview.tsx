...
import { lsBlocksAtom } from '../../core/atoms/blocks';
...
const WebPreview = () => {
  const { websiteId, pageName } = useParams<{ websiteId: string, pageName: string }>();
  const [blocks] = useAtom(lsBlocksAtom);
  const [_, setBlocks] = useState<any[]>([]);
...
const { data, error } = await supabase
  ...
  const html = ReactDOM.renderToString(<RenderChaiBlocks lang={selectedLang} blocks={blocks} />);
  return (
...
export default () => (
  <Routes>
    <Route path="/website/:websiteId/:pageName/preview" element={<WebPreview />} />
  </Routes>
);
